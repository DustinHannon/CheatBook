...
/** Account & sign-in: Entra identity card, read-only identity table, sign-out action. */
export const AccountSection: React.FC<AccountSectionProps> = ({ me }) => {
  ...
  const signOut = async () => {
    ...
    showToast('Signing you out of CheatBook.', 'info');
  ...
  return (
    <>
      ...
      >
        ...
        >
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--success)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2l8 4v5c0 5-3.4 8.5-8 11-4.6-2.5-8-6-8-11V6z" /><path d="M9.5 12.5l1.8 1.8 3.5-3.6" /></svg>
        </div>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 13.5, fontWeight: 700, color: 'var(--success)' }}>Signed in with Microsoft Entra ID</div>
          <div style={{ fontSize: 11.5, color: 'var(--text-3)', marginTop: 2 }}>Single sign-on · managed by your IT administrator</div>
        </div>
      ...
      >
        <div style={{ fontSize: 13, fontWeight: 800, color: 'var(--danger)', marginBottom: 4 }}>Sign out</div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <div style={{ flex: 1, fontSize: 12, color: 'var(--text-2)', lineHeight: 1.5 }}>
            End your CheatBook session on this device. Your account, spaces, and authored notes stay exactly as they are.
          </div>
          <button
            type="button"
      ...
      <ConfirmDialog
        open={confirmOpen}
        title="Sign out of CheatBook?"
        message="You'll be returned to the sign-in screen and your session on this device will end. Your account and notes are unaffected — sign back in any time."
