...
A break-glass account is already provisioned in Supabase Auth and auto-joined to the
team as an admin. On the login screen, open **"Use a break-glass account"** and sign in:
...
4. **API permissions:** Microsoft Graph → delegated `email`, `openid`, `profile`, `User.Read` → Grant admin consent.
...
redirect flow; new users are auto-joined to the MorganWhiteGroup team on first sign-in.
